/**
 * A simple in-memory private-state provider for the browser. Ported, as-is
 * and generic, from the official Midnight bulletin-board example's
 * `bboard-ui/src/in-memory-private-state-provider.ts` (Apache-2.0).
 *
 * The Midnight SDK's LevelDB-backed private state provider is Node-only —
 * the official browser example uses this in-memory provider instead, which
 * means private state (participant/organizer secret keys, in Wave 1's case)
 * does not survive a page reload. That is a known Wave 1 limitation, not an
 * oversight: matching the verified browser reference is safer than
 * inventing untested persistence.
 */
import type {
  ContractAddress,
  SigningKey,
